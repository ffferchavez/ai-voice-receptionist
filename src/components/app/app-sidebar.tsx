          ...
          return (
            <Link
              ...
              className={[
                ...
                active
                  ? "bg-white text-ui-text ring-1 ring-ui-line"
                  : "text-ui-muted hover:bg-white/70 hover:text-ui-text",
              ].join(" ")}
            >
              <Icon />
              {!iconOnly && (
...
function SidebarUserFooter({
  ...
  const safe = (initials || "SD").slice(0, 2).toUpperCase();
  ...
  if (iconOnly) {
    return (
      <div
        className={`shrink-0 border-t border-ui-line bg-ui-bg ${bottomSafe}`}
      >
        <div className="flex flex-col items-center gap-2 px-2 py-3">
          <span
            className="inline-flex size-10 items-center justify-center rounded-md bg-ui-surface text-[11px] font-semibold text-ui-text ring-1 ring-ui-line"
            title={shortLabel}
            aria-label="Account"
          >
            {safe}
  ...
  return (
    ...
    >
      <div className="p-3">
        <div className="rounded-md border border-ui-line bg-white p-3">
          <div className="flex gap-3">
            <div className="inline-flex size-10 shrink-0 items-center justify-center rounded-md bg-ui-surface text-[11px] font-semibold tracking-tight text-ui-text ring-1 ring-ui-line">
              {safe}
            </div>
  ...
  return (
    <>
      <aside
        ...
        className={[
          "relative hidden h-full min-h-0 shrink-0 flex-col border-r border-ui-line bg-ui-bg transition-[width] duration-250 ease-in-out md:flex",
          desktopExpanded
            ? "w-[248px]"
            : "w-[76px]",
        ].join(" ")}
      ...
      <aside
        className={[
          "fixed inset-y-0 left-0 z-50 flex w-[min(300px,90vw)] max-w-full flex-col border-r border-ui-line bg-ui-bg shadow-[8px_0_24px_rgba(51,38,90,0.14)] transition-transform duration-300 ease-in-out md:hidden",
          mobileOpen ? "translate-x-0" : "-translate-x-full",
        ].join(" ")}
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-ui-line px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
          ...
          <button
            type="button"
            onClick={() => onMobileOpenChange(false)}
            className={`${navLinkFocus} inline-flex size-11 shrink-0 items-center justify-center text-ui-muted transition-colors hover:bg-white hover:text-ui-text`}
